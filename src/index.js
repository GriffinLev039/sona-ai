...
//Outdated, content will be moved to .txt file within /txt/
...
const gif3List = ['https://tenor.com/view/cat-generator-cat-generator-holy-shit-cute-cat-gif-25893306', 'https://media.discordapp.net/attachments/810336789465530378/1021472450648748082/considerabledelay.gif?ex=65b7dfda&is=65a56ada&hm=3a8baaebfd69503291c33c26298151b7d9306b712a5c95ddb3413d1a5350777a&'];let name = 'you';


//Imports files and creates list, removing blank end space created by nano.
const fs = require('fs');
const sonaTalk = fs.readFileSync('./quotes.txt', 'utf-8').split(/\r?\n/);
const sonaURL = fs.readFileSync('./gifs.txt', 'utf-8').split(/\r?\n/);
sonaTalk.pop();
sonaURL.pop();
console.log(sonaTalk);
console.log(sonaURL);

//Creates a writing stream to add Sona quotes to a file. 
const writer = fs.createWriteStream('../txt/sonaQuotes.txt');
...
client.on('messageCreate', async message => {
    ...
    console.log(rndm);
    if (message.content.toLowerCase().includes('sona') || message.author.id === '367853307097513996' && message.author.id !== '1198505072691269652') { //Does not reply if the sender is itself
        ...
        switch (message.content.toLowerCase().trim().startsWith()) {
            case '!addquote':
                if (message.content.replace('!addQuote', '').trim() !== '') {
                    let sonaStr = message.content.replace('!addQuote', '').replaceAll("'", "").replaceAll('"', '').replaceAll('/', '');
                    writer.write(sonaStr);
                }
                break;
            case '!listquote':
                await message.reply(fs.readFileSync('../txt/sonaQuotes.txt'));
                break;
            case '!addgif':
                await message.reply("This isn't implemented yet!");
                break;
            case '!listgif':
                await message.reply("This isnt implemented yet!");
            default:
                break;
        }
    }
